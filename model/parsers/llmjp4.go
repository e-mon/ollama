...
// LLMJP4Parser handles LLM-jp-4 models. They use the harmony format with two
// differences, so the stream is adapted here rather than in the harmony package:
//   - the tokenizer decodes a space after every special token
//     ("<|channel|> analysis<|message|> ..."). One space is removed after each
//     special token before the text reaches the harmony parser; the rule is
//     lossless because an intended leading space arrives as two.
//   - LLM-jp-4.1 emits several tool calls per turn, closing all but the last
//     with <|end|>, so a call is completed at each message end instead of at
